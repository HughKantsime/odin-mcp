...
const printers = loadJson("printers.json");
const filaments = loadJson("filaments.json");
...
server.registerTool("calculate_print_cost", {
    ...
    description: "Calculate the true cost of a 3D printed part including material, electricity, depreciation, labor, and failure rate",
    inputSchema: {
    ...
    const { printer_model, filament_type, filament_cost_per_kg, print_weight_grams, print_time_hours, electricity_rate_kwh, labor_rate_per_hour, failure_rate_pct, } = args;
    ...
    if (printer_model) {
        const found = printers.find((p) => p.name.toLowerCase().includes(printer_model.toLowerCase()) ||
    ...
    const electricityCost = (wattage / 1000) * print_time_hours * electricity_rate_kwh;
    ...
    const failureBuffer = subtotal * (failure_rate_pct / 100 / (1 - failure_rate_pct / 100));
...
server.registerTool("compare_farm_software", {
    ...
    description: "Compare 3D print farm management platforms: ODIN vs SimplyPrint vs 3DPrinterOS vs OctoFarm vs Obico",
    inputSchema: {
        printer_count: z
            .number()
        ...
        protocols_needed: z
            .array(z.string())
            .optional()
            .describe("Printer protocols needed (bambu-mqtt, moonraker, prusalink, elegoo-sdcp)"),
    ...
    const { printer_count, protocols_needed, self_hosted_required, budget_monthly } = args;
    const scored = competitors.map((c) => {
        ...
        if (protocols_needed && protocols_needed.length > 0) {
            const matched = protocols_needed.filter((p) => c.protocols.includes(p));
        ...
        if (features.unlimitedPrinters)
            score += 10;
        // Bonus for AI detection
        if (c.aiDetection === true)
            score += 5;
    ...
    lines.push(``);
    // Table header
    lines.push(`| Rank | Platform       | Monthly Cost | Protocols | Self-Hosted | Features | Score |`);
    lines.push(`|------|---------------|-------------|-----------|-------------|----------|-------|`);
    scored.forEach((s, i) => {
        ...
        const protocols = s.competitor.protocols.length > 0
            ? s.competitor.protocols.length.toString()
            : "0";
        const selfHosted = s.competitor.selfHosted ? "Yes" : "No";
        const features = `${s.featureCount}/14`;
        lines.push(`| #${i + 1}   | ${name} | ${cost} | ${protocols.padEnd(9)} | ${selfHosted.padEnd(11)} | ${features.padEnd(8)} | ${s.score.toFixed(0).padStart(4)}  |`);
    });
    lines.push(``);
    lines.push(`--- Details ---`);
    lines.push(``);
...
server.registerTool("recommend_printer_for_farm", {
    ...
    description: "Recommend the best 3D printer for a print farm based on use case, budget, and fleet requirements",
    inputSchema: {
        use_case: z
            .string()
            .describe("Primary use case (production, prototyping, education, dental, miniatures, cosplay)"),
    ...
    const scored = candidates.map((p) => {
        ...
        switch (priority) {
            case "speed":
                ...
                if (p.printSpeed >= 500)
                    reasons.push("High speed printing");
                break;
            case "quality":
        ...
        if (p.farmScore >= 85)
            reasons.push(`Excellent farm score (${p.farmScore})`);
        ...
        if (existing_fleet && existing_fleet.length > 0) {
            const existingManufacturers = existing_fleet.map((name) => {
                const found = printers.find((pr) => pr.name.toLowerCase().includes(name.toLowerCase()) ||
    ...
    lines.push(`Manage them all with ODIN — https://runsodin.com`);
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
// -------------------------------------------------------------------
...
server.registerTool("estimate_farm_capacity", {
    ...
    description: "Estimate monthly production capacity and utilization of a 3D print farm",
    inputSchema: {
        printers: z
            .array(z.object({
    ...
    const fleet = fleetInput.map((entry) => {
        // Look up printer
        const found = printers.find((p) => p.name.toLowerCase().includes(entry.model.toLowerCase()) ||
        ...
        const printsPerDayPerUnit = Math.floor(availableMinutesPerDay / printCycleMinutes);
    ...
    const totalDailyPrints = fleet.reduce((sum, f) => sum + f.totalPrintsPerDay, 0);
    const totalMonthlyPrints = fleet.reduce((sum, f) => sum + f.totalPrintsPerMonth, 0);
    // Find bottleneck (lowest prints per day per unit)
    const bottleneck = fleet.reduce((min, f) => f.printsPerDayPerUnit < min.printsPerDayPerUnit ? f : min);
    // Overall utilization
    const totalAvailableMinutes = fleet.reduce((sum, f) => sum + f.availableMinutesPerDay * f.count, 0);
    const totalPrintingMinutes = fleet.reduce((sum, f) => sum + f.printsPerDayPerUnit * avg_print_time_hours * 60 * f.count, 0);
    ...
    lines.push(`${bottleneck.printerName} is the constraint at ${bottleneck.printsPerDayPerUnit} prints/day/unit.`);
    if (overallUtilization > 85) {
        lines.push(``);
        lines.push(`Your farm is running near capacity. Consider adding printers to handle demand spikes.`);
    }
    else if (overallUtilization < 50) {
        lines.push(``);
        lines.push(`Your farm has significant spare capacity. Consider extending operating hours or taking on more orders before adding printers.`);
    }
    lines.push(``);
    lines.push(`Plan your expansion at https://runsodin.com/tools/capacity-planner`);
    return { content: [{ type: "text", text: lines.join("\n") }] };
});
// Start the server
async function main() {
